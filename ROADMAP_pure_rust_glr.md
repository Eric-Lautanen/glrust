...
//   1. The ACTION table  [state × terminal   → shift/reduce/accept/error]
//   2. The GOTO table    [state × nonterminal → new_state]
...
storing a full dense row for every state is correct but uses ~10× more memory
for real grammars (the C grammar has 2,015 states × 360 symbols).
...
4. ε-rule handling: reductions of zero-length productions must be
   ...
   (Right-Nulled GLR) to handle ε-rules correctly and efficiently.
```
...
handles ε-rules with O(n³) worst-case complexity and is the algorithm used by
rustemo (which can be studied as a reference implementation). Tomita's original
Algorithm 1 fails on grammars with ε-rules.
...
7. Emit compressed parse table (large-state + small-state split, see §0.2)
...
1. **ε-rules in GLR** — Productions that reduce to the empty string (ε) cause
...
6. **Performance** — C's `goto`-based state machine dispatcher is hard to beat.
   ...
   matter at this level. Expect 1.5–2× C performance initially; optimizing to
...
|---------|------|-------|----------|-------------|---------------|-------------------------------|
| **lalrpop** | LR(1) proc-macro | ~4k | LALR(1) opt | No | No | No (own DSL) |
| **pest** | PEG proc-macro | ~5k | No (PEG) | No | No | No (own `.pest` files) |
| **rust-peg** | PEG proc-macro | ~1.7k | No (PEG) | No | No | No (own DSL) |
| **grmtools/lrpar** | LR/GLR build.rs | ~800 | **Yes** (GLR mode) | No | No | Yacc `.y` files |
| **rustemo** | LR/GLR build.rs | ~250 | **Yes** (RNGLR + SPPF) | No | No | Own grammar DSL |
| **RustyLR** | IELR(1)/LALR(1)/GLR proc-macro | ~400 | **Yes** | No | No | Own bison-like DSL |
| **glr-parser** (crate) | GLR generator | <100 | **Yes** | No | No | No |
| **tree-sitter** | C GLR library | ~25.7k (Rust+C) | Yes | **Yes** | **Yes** | Yes (200+ grammars) |
...
> writing is v0.26.2 (December 2025). The star count for the combined
...
GLR, which correctly handles ε-rules without Tomita's edge cases), produces a
...
2015 crate). Consider `treacle` (tree + oracle), `arborist`, `glrust`, or
`sylvan` as the top-level crate name. Reserve the name early.
...
> latest) in `Cargo.toml`. The Rust bindings crate minimum Rust version is
> **1.77.0**.
...
| **RNGLR** | Right-Nulled GLR — a variant of GLR (Scott & Johnstone, 2006) that correctly handles ε-rules without looping. The recommended algorithm for this project |
...
| **ε-rule / epsilon-rule** | A production with an empty right-hand side: `Nonterminal → ε`. These cause infinite loops in naive GLR; RNGLR handles them correctly |
| **Parse table** | Two tables: ACTION `[state × terminal → shift/reduce/accept/error]` and GOTO `[state × nonterminal → new_state]`. Often stored merged |
...
|------|---------------|---------------|
| ε-rule in the start production (`S → ε`) | Parser must accept empty input and produce a tree with a single empty node | `parse("")` → tree with `root.start_byte == root.end_byte == 0` |
| Deeply nested right-recursion (`A → "a" A \| ε`) | If reductions are processed recursively (DFS on the GSS), right-recursion causes O(n) recursive calls → stack overflow. Must use an iterative worklist for reductions. | Input of 100,000 `"a"` tokens — no stack overflow, worklist stays bounded |
| Deeply nested left-recursion (`A → A "a" \| ε`) | GSS must share nodes; naïve impl is O(n²) space | Input of 100,000 `"a"` tokens — RSS bounded |
...
| Repeated ε-reduce cycle | GLR step could loop on `A → ε; B → A; A → B` | Fixed-point detection terminates after one full pass |
| Unicode in identifiers | Token `start_byte` ≠ `start_col` when input has multi-byte chars | `"let π = 3"` — identifier node byte-span covers the 2-byte `π` correctly |
...
| Edit crosses a scanner state boundary | Scanner was in state S₁ before edit, new text changes it to S₂ | Python: inserting a blank line between two indented blocks must re-scan INDENT/DEDENT from that line |
